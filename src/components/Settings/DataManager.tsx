...
export function DataManager({ views, onRefresh, onToast }: DataManagerProps) {
  ...
  const openExportPicker = async () => {
    try {
      const assetViews = await getTransport().invoke<{ id: number; name: string; view_type: string; is_default: boolean }[]>('list_views', { viewType: 'asset' });
      const dexViews = await getTransport().invoke<{ id: number; name: string; view_type: string; is_default: boolean }[]>('list_views', { viewType: 'dex' });
  ...
  const handleExport = async () => {
    ...
    try {
      const rustData = await getTransport().invoke<RustExportData>('export_data');
      ...
      await getTransport().invoke('export_file', {
  ...
  const handleImport = async () => {
    let raw: string;
    try {
      raw = await getTransport().invoke<string>('import_file');
    } catch { return; }
    ...
    try {
      const [imported, skipped] = await getTransport().invoke<[number, number]>('import_data', { data: rustImportData });
  ...
  const handleReset = async () => {
    ...
    try {
      await getTransport().invoke('reset_all_data');
