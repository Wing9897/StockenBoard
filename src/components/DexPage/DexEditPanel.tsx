...
export function DexEditPanel({ subscription, providers, isCustomView, onSave, onRemove, onClose }: DexEditPanelProps) {
  ...
  const handleLookup = async () => {
    ...
    try {
      const info = await getTransport().invoke<{ token0_address: string; token0_symbol: string; token1_address: string; token1_symbol: string }>(
  ...
  const handleSave = async () => {
    ...
    try {
      await getTransport().invoke('fetch_asset_price', { providerId: editProvider, symbol: testSymbol });
    } catch (err) {
