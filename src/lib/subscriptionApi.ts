...
export async function addAssetSubscription(
  ...
  // 先驗證 symbol 有效性
  await getTransport().invoke('fetch_asset_price', { providerId: pid, symbol: storedSymbol });
  // 再新增訂閱（走 Rust IPC）
  await getTransport().invoke<number>('add_subscription', {
    subType: 'asset',
    symbol: storedSymbol,
...
export async function addAssetSubscriptionBatch(
  ...
  for (const [pid, group] of groups) {
    ...
    try {
      const results = await getTransport().invoke<{ symbol: string }[]>('fetch_multiple_prices', { providerId: pid, symbols });
  ...
  if (allValid.length > 0) {
    try {
      ...
      const result = await getTransport().invoke<{ succeeded: string[]; failed: string[]; duplicates: string[] }>(
...
export async function updateAssetSubscription(
  ...
  await getTransport().invoke('update_subscription', {
    id: sub.id,
    symbol: storedSymbol,
...
export async function hasApiKey(providerId: string): Promise<boolean> {
  try {
    return await getTransport().invoke<boolean>('has_api_key', { providerId });
...
export async function removeSubscription(id: number): Promise<void> {
  await getTransport().invoke('remove_subscription', { id });
}

/** 批量刪除訂閱 */
export async function removeSubscriptions(ids: number[]): Promise<void> {
  if (ids.length === 0) return;
  await getTransport().invoke('remove_subscriptions', { ids });
}
