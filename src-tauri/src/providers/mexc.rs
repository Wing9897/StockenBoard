...
fn parse_mexc_ticker(symbol: &str, item: &serde_json::Value) -> AssetData {
    ...
    AssetDataBuilder::new(symbol, "mexc")
        ...
        .change_percent_24h(pf("priceChangePercent").map(|p| p * 100.0))
...
impl DataProvider for MexcProvider {
    fn info(&self) -> ProviderInfo { get_provider_info("mexc").unwrap() }
    ...
    async fn fetch_price(&self, symbol: &str) -> Result<AssetData, String> {
        ...
        let data: serde_json::Value = self.client.get(&url)
            .send().await.map_err(|e| format!("MEXC 連接失敗: {}", e))?
            .error_for_status().map_err(|e| format!("MEXC API 錯誤: {}", e))?
            .json().await.map_err(|e| format!("MEXC 解析失敗: {}", e))?;
    ...
    async fn fetch_prices(&self, symbols: &[String]) -> Result<Vec<AssetData>, String> {
        ...
        let arr: Vec<serde_json::Value> = self.client.get(url)
            .send().await.map_err(|e| format!("MEXC 批量連接失敗: {}", e))?
            .json().await.map_err(|e| format!("MEXC 批量解析失敗: {}", e))?;
