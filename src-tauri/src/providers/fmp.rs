...
impl FMPProvider {
    ...
    fn parse_quote(symbol: &str, q: &serde_json::Value) -> AssetData {
        AssetDataBuilder::new(symbol, "fmp")
            ...
            .extra_f64("開盤價", q["open"].as_f64())
            .extra_f64("前收盤價", q["previousClose"].as_f64())
            .extra_f64("52週高", q["yearHigh"].as_f64())
            .extra_f64("52週低", q["yearLow"].as_f64())
            .extra_f64("本益比", q["pe"].as_f64())
            .extra_f64("每股盈餘", q["eps"].as_f64())
            .extra_str("公司名稱", q["name"].as_str())
            .build()
    }
}
