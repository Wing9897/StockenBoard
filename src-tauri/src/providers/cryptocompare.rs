...
impl CryptoCompareProvider {
    ...
    fn parse_coin(symbol: &str, base: &str, data: &serde_json::Value) -> Result<AssetData, String> {
        ...
        if raw.is_null() {
            return Err(format!("CryptoCompare 找不到: {} (查詢: {})。格式: BTC, ETH", symbol, base));
        }
...
impl DataProvider for CryptoCompareProvider {
    ...
    async fn fetch_price(&self, symbol: &str) -> Result<AssetData, String> {
        ...
        let url = format!("https://min-api.cryptocompare.com/data/pricemultifull?fsyms={}&tsyms=USD", base);

        let data: serde_json::Value = self.build_request(&url)
            .send().await.map_err(|e| format!("CryptoCompare 連接失敗: {}", e))?
            .error_for_status().map_err(|e| format!("CryptoCompare API 錯誤: {}", e))?
            .json().await.map_err(|e| format!("CryptoCompare 解析失敗: {}", e))?;
    ...
    async fn fetch_prices(&self, symbols: &[String]) -> Result<Vec<AssetData>, String> {
        ...
        let url = format!("https://min-api.cryptocompare.com/data/pricemultifull?fsyms={}&tsyms=USD", fsyms);

        let data: serde_json::Value = self.build_request(&url)
            .send().await.map_err(|e| format!("CryptoCompare 批量連接失敗: {}", e))?
            .error_for_status().map_err(|e| format!("CryptoCompare API 錯誤: {}", e))?
            .json().await.map_err(|e| format!("CryptoCompare 批量解析失敗: {}", e))?;
