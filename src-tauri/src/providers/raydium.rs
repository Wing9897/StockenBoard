...
impl RaydiumProvider {
    ...
    fn base_url(&self) -> &str {
        self.api_url
            .as_deref()
            .unwrap_or("https://api-v3.raydium.io")
    }
    ...
    fn parse_symbol(symbol: &str) -> Result<(&str, &str, &str), String> {
        ...
        if parts.len() != 3 {
            return Err(format!(
                "Invalid Raydium symbol format '{}', expected 'pool:tokenFrom:tokenTo'",
                symbol
            ));
        }
...
impl DataProvider for RaydiumProvider {
    ...
    async fn fetch_price(&self, symbol: &str) -> Result<AssetData, String> {
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| format!("Raydium request failed: {}", e))?;
        if !resp.status().is_success() {
            return Err(format!("Raydium API error: HTTP {}", resp.status()));
        }

        let body: RaydiumPoolResponse = resp
            .json()
            .await
        ...
        let mint_a_addr = pool
            .mint_a
            .as_ref()
            .and_then(|m| m.address.as_deref())
            .unwrap_or("");
        let mint_b_addr = pool
            .mint_b
            .as_ref()
            .and_then(|m| m.address.as_deref())
            .unwrap_or("");
    ...
    async fn fetch_prices(&self, symbols: &[String]) -> Result<Vec<AssetData>, String> {
        ...
        let mut pool_map: std::collections::HashMap<String, Vec<(String, String, String)>> =
            std::collections::HashMap::new();
        for sym in symbols {
            let (pool, tf, tt) = Self::parse_symbol(sym)?;
            pool_map.entry(pool.to_string()).or_default().push((
                sym.clone(),
                tf.to_string(),
                tt.to_string(),
            ));
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| format!("Raydium batch request failed: {}", e))?;
        if !resp.status().is_success() {
            return Err(format!("Raydium API error: HTTP {}", resp.status()));
        }
        ...
        let pools: Vec<RaydiumPool> = body
            .data
            .unwrap_or_default()
            .into_iter()
            .flatten()
        ...
        for (i, pool) in pools.iter().enumerate() {
            ...
            for (sym, token_from, token_to) in requests {
                ...
                results.push(
                    AssetDataBuilder::new(sym, "raydium")
                        .price(usd_price)
                        .volume(pool.day.as_ref().and_then(|d| d.volume))
                        .extra_f64("pool_tvl", pool.tvl)
                        .extra_f64("amount_out", Some(amount_out))
...
impl DexPoolLookup for RaydiumProvider {
    async fn lookup_pool(&self, pool_address: &str) -> Result<DexPoolInfo, String> {
        ...
        let pool = body
            .data
            .and_then(|d| d.into_iter().flatten().next())
